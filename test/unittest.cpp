...
protected:
    virtual void SetUp() {
        ...
        u1->damage_player_or_enemy("game_enemies",5,100);
        i1->add_Player(100);
        u1->add_experience(100,1000);
        u1->add_gold(100,50);
    }
    virtual void TearDown() {
        delete u1;
...
TEST_F(UpdateTest, TESTFIXTURE) {
    ASSERT_EQ(s1->get_one_entry("game_enemies", "health", 100), 95) << "These should match!";
    ASSERT_EQ(s1->get_one_entry("players","experience",100), 1000) << "These should match";
    ASSERT_EQ(s1->get_one_entry("players","gold",100), 50) << "These should match";
}
...
TEST_F(InsertTest, TESTFIXTURE) {
    ...
    ASSERT_EQ(s1->get_one_entry("inventory","damage",20),50) << "These should match";
...
TEST_F(SelectTest, TESTFIXTURE) {
    ...
    std::vector<std::string> inventory = {"20", "Katana", "50"};
    ...
    for (int i=0; i<3; i++) {
        ASSERT_EQ(s1->get_one_row_string("inventory","Katana")[i], inventory[i]) << "These should match!";
    }
    for (int i=0; i<3; i++) {
    ...
    void SetUp() override{
        ...
        input.reserve(11);
         ...
         for(int i=0;i<input.size();i++) {
             std::cout << input[i].get_col() <<input[i].get_row()<< std::endl;
         }
     h=new Hallway(input, 7,4,2,0);
        std::cout << h->get_offsetc()<< std::endl;
    }
